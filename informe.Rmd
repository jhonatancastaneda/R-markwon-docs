...
Para $: \beta_1$ es:
```{r,echo=TRUE,eval=TRUE}
summary(reg)$coefficients[[5]]


```
Como $t=`r summary(reg)$coefficients[[5]]`$ es `r tex<-"menor"; if(summary(reg)$coefficients[[5]]>qt(0.975,(nrow(data)-2))) tex<-"mayor"; tex` que $t_{`r (nrow(data)-2)`}(\frac{\alpha}{2})= `r qt(0.95,(nrow(data)-2))`$
`r tex<-"no rechazo"; if(summary(reg)$coefficients[[5]]>qt(0.95,(nrow(data)-2))) tex<-"rechazo"; tex`
$H_0: \beta_1=0$.

Para $: \beta_2$ es:
```{r,echo=TRUE,eval=TRUE}
summary(reg)$coefficients[[6]]


```
Como $t=`r summary(reg)$coefficients[[6]]`$ es `r tex<-"menor"; if(summary(reg)$coefficients[[6]]>qt(0.975,(nrow(data)-2))) tex<-"mayor"; tex` que $t_{`r (nrow(data)-2)`}(\frac{\alpha}{2})= `r qt(0.95,(nrow(data)-2))`$
`r tex<-"no rechazo"; if(summary(reg)$coefficients[[6]]>qt(0.95,(nrow(data)-2))) tex<-"rechazo"; tex`
$H_0: \beta_2=0$.
